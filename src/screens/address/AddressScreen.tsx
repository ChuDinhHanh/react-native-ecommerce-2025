...
const AddressScreen = () => {
    ...
    const [createNewAddress, { isError, isLoading, isSuccess, reset, data, error }] = useCreateNewAddressMutation();

    const validationSchema = Yup.object().shape({
        nameGet: Yup.string().required('Tên người nhận hàng là bắt buộc'),
        phoneGet: Yup.string().required('Số điện thoại là bắt buộc').min(10, 'Số điện thoại phải có ít nhất 10 chữ số'),
        description: Yup.string().optional(),
        addressShow: Yup.string().required('Địa chỉ giao hàng là bắt buộc'),
    });

    const handleCreateNewAddress = useCallback((values: LocationMarkAndAddress, setFieldValue: any) => {
        setVisible(false);
                        ...
                        {({ handleChange, handleBlur, handleSubmit, values, errors, touched, setFieldValue }) => (
                            <>
                                <CustomTextInput
                                    ...
                                    suffix={<Octicons name='feed-person' size={Variables.ICON_SIZE_SMALL} />}
                                ...
                                <CustomTextInput
                                    title='Số điện thoại'
                                    suffix={<MaterialIcons name='local-phone' size={Variables.ICON_SIZE_SMALL} />}
                                ...
                                <CustomTextInput
                                    title='Lời nhắn'
                                    suffix={<Octicons name='pencil' size={Variables.ICON_SIZE_SMALL} />}
                                ...
                                <TextButtonComponent
                                    ...
                                    borderRadius={5}
                                    spaceAffix={16}
                                    iconOrImageAffix={<FontAwesome5 size={Variables.ICON_SIZE_SMALL} name="map-marked-alt" />}
                                    title={<TextComponent text='Chọn địa chỉ giao hàng' color={Colors.BLACK} />}
                                    onPress={() => setVisible(true)}
                                />
                                <TextComponent text={values.addressShow} color={Colors.BLACK} />
                                {errors.addressShow && touched.addressShow && (
                                    <TextComponent fontSize={Variables.FONT_SIZE_ERROR_TEXT} text={errors.addressShow} color={Colors.RED} />
                                )}

                                <RowComponent marginVertical={verticalScale(16)} justifyContent='flex-start' alignItems='center'>
                                    <CheckBox
                                        disabled={false}
                                ...
                                <TextButtonComponent
                                    ...
                                    title={<TextComponent fontWeight='bold' fontSize={Variables.FONT_SIZE_BUTTON_TEXT} color={Colors.WHITE} text={"Xác nhận địa điểm giao hàng"} />}
                                />
                                <Portal>
                                    <Modal visible={visible} onDismiss={() => setVisible(false)} contentContainerStyle={{ flex: 1 }}>
                                        <MapComponent onPress={(values) => handleCreateNewAddress(values, setFieldValue)} />
                                    </Modal>
                                </Portal>
                            </>
                        )}
