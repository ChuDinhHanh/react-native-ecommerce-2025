...
const VerificationWithEmailScreen = () => {
  ...
  const route =
    useRoute<RouteProp<RootStackParamList, 'VERIFY_EMAIL_SCREEN'>>();
  const token = route.params.token;
  const email = route.params.email;
  const [
    checkVerifyToken,
    {data: dataCheckVerifyToken, error, isError, isLoading, isFetching},
    ...
    <ContainerComponent isFull isScrollEnable>
      ...
      <SessionComponent>
        <TextComponent
          ...
          text={t('VerificationWithEmailScreen.title1')}
          color={Colors.BLACK}
        />
        <TextComponent
          fontSize={scale(15)}
          text={`${t('VerificationWithEmailScreen.title2')} ${email}`}
          color={Colors.GREY1}
        />
        <SpaceComponent height={30} />
        <TextComponent
          fontSize={scale(15)}
          text={t('VerificationWithEmailScreen.title3')}
          color={Colors.GREY1}
        />
        <RowComponent>
          <TextComponent
            fontSize={scale(15)}
            text={t('VerificationWithEmailScreen.title4')}
            color={Colors.GREY1}
          />
          ...
          <TextButtonComponent
            ...
            title={
              <TextComponent
                ...
                text={t('VerificationWithEmailScreen.textButtonResendEmail')}
              />
            }
          />
        </RowComponent>
