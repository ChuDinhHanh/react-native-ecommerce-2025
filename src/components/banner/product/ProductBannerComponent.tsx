...
const ProductBannerComponent = (props: Props) => {
  ...
  useEffect(() => {
    if (data) {
      if (isFocusing && autoScroll && data.length != 0) {
        startAutoScroll();
      } else {
        stopAutoScroll();
      }
  ...
  const renderItem = (item: string, index: number) => {
    return (
      <Pressable
        ...
        onPress={() => {
          console.log('click banner', index);
        }}>
        <View
          style={{
            width: widthOfBanner,
          ...
          {Boolean(item) && (
            <FastImage
              style={[styles.bannerImages, {height, borderRadius}]}
              source={{
                uri: item.includes('http')
                  ? `${item}`
                  : `${SERVER_ADDRESS}api/get/image/${item}`,
              }}
            />
          )}
        </View>
